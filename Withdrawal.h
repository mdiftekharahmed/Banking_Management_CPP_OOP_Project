...
public:
    Withdrawal(Database& database, int accId, double amt)
        : Transaction(database, accId, amt, "WITHDRAWAL") {}
    ...
    bool execute() override {
        try {
            ...
            // Record transaction
            string recordTxn = "INSERT INTO transactions (account_id, type, amount) VALUES (" + to_string(accountId) + ", 'WITHDRAWAL', " + to_string(amount) + ");";
            db.executeQuery(recordTxn);
            
            return true;
        } catch (const exception& e) {
            cerr << "Withdrawal Error: " << e.what() << endl;
            return false;
        }
    }
};
